...
This is where butterfly is used in a data processing pipeline, to prevent unintended DOI invalidation through publishing datasets with unnoticed changes. The full methods are described in this [article](https://thomaszwagerman.github.io/butterfly/articles/butterfly_in_pipeline.html) and source code is available in the [asli-pipeline repository](https://github.com/antarctica/asli-pipeline) [@zwagerman_2024]
...
```{r 100hz_sampling, echo=FALSE, out.width="100%", fig.cap="An illustration of intentional non-continuous measurement, when weight is detected on the weighbridge."}
knitr::include_graphics("img/100hz_sampling.png")
```
...
```{r crossing_example, echo=FALSE, out.width="100%", fig.cap="An example of an individual penguin crossing, the continuous crossing was identified and grouped using `timeline_group()`."}
knitr::include_graphics("img/crossing_example.png")
```
