...
When ... specific version of data used to

maintain data provenance.

When using other data to generate your own, it is crucial to reference the exact version of the data used, in order to maintain data provenance.

In other instances it may be required to revise data publications, due to the discovery of an inconsistency or error.

But what if you are not aware of upstream changes to your input data?

Here we present butterfly, an R package for the verification of continually updating timeseries data where we expect new values, but want to ensure previous data remains unchanged.
